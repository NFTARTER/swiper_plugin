...
## Swiper 작성 준비
0. `head` 태그 안 `title` 아래 Swiper사용을 위한 CDN연결
...
* 1. loop - 슬라이드 끝-> 시작 반복 true, false
* 2. effect - fade : 서서히 사라지면서 트랜지션, cube : 큐브모양이 입체적으로 회전하면서 화면 전환, flip : 평면적으로 화면이 뒤집혀지는 효과
* 3. direction - 'vertical' 수직 / 'horizontal' 수평으로 슬라이드 방향이 넘어간다. (디폴트 값은 수평이다) + mousewheel:true 마우스휠을 올리고 내리면서 화면을 전환
* 4. autoplay - 'delay' - 1000 = 1초이다.  / 'pauseOnMouseEnter' = 마우스를 올리면 일시정지 / 'disableOnInteraction' = 마우스 상호작용 후에 멈출건지, 움직일건지에 대한 옵션
## Swiper 선택요소(페이지번호, 이전다음버튼, 스크롤바) 주의사항
* 기본작성 위치는 `swiper-wrapper`의 다음 형제 위치
    * `swiper`의 첫번째 자식(swiper-wrapper) 두번째자식(선택요소들)
    * **절대 `swiper-slide`와 형제,자식,자손위치에 배치하지 않기**
* 기본순서 : `html -> js swiper options 세팅 -> (선택)css`
**아래 선택요소들 태그는 자식, 자손으로 다른태그 삽입 금지**
...
### 기존 Swiper-slide 겹치는 디자인을 하고 싶을때 
...
* `div.swiper`에 기본CCS로 `overflow:hidden` 있으므로 그 값을 `visible`로 바꾸면 선택요소 뿐 아닌 `swiper-slide` 들도 모두 보이기 때문에 원하는 레이아웃을 만들 수 없음.
